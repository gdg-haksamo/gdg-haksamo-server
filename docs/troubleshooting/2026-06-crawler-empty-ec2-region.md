# 메뉴 크롤러가 200을 반환하지만 DB에 아무것도 저장 안 됨
...
> ⚠️ **함정 — `\` 줄바꿈 복붙 사고**: 명령을 백슬래시(`\`)로 여러 줄에 걸쳐 붙여넣으면, 백슬래시 뒤에 공백이 섞여 줄 연속이 깨진다. 그러면 `curl`이 **URL 없이 실행되어 `HTTP=000`(가짜 실패)** 가 뜨고, URL 줄은 `-bash: https://...: No such file or directory` 로 따로 실행된다. **000을 "연결 실패"로 오해하기 쉽다.** 위 예시처럼 **반드시 한 줄로 실행**해 진짜 결과를 봐야 한다.
...
> **EC2는 앞으로 서울 리전 유지 필수** — 해외 리전이면 크롤이 구조적으로 안 됨.
