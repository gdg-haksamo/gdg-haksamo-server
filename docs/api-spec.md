...
- 400 `U008`(restaurantId 누락) / 404 `U010`(식당 없음) / 400 `C001`(검증 실패) / 409 `U001`(이메일 중복)
...
- 400 `U008`(RESTAURANT_ADMIN인데 식당 미지정) / 404 `U010`(식당 없음) / 400 `U009`(본인 계정 변경 불가) / 404 `U002`
...
|---|---|---|
| C001 | 400 | 입력값 검증 실패 |
| C003 | 409 | 동시 요청 등 DB 제약 충돌(잠시 후 재시도) |
| A001 | 401 | 인증 필요 |
...
| U010 | 404 | 지정한 식당 없음(식당 운영자 발급/권한변경 시) |
